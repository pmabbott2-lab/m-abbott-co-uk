...
export const getMyRole = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    // auto-granted the advisor role the first time they sign in. This seeds the
    // very first admin, who can then manage everyone else from inside the app.
    const email = (context.claims as { email?: string }).email?.trim().toLowerCase();
    const adminEmails = (process.env.ADMIN_EMAILS ?? "")
      .split(",")
      .map((e) => e.trim().toLowerCase())
      .filter(Boolean);
    if (email && adminEmails.includes(email) && !roles.includes("advisor")) {
      const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
      await supabaseAdmin
        .from("user_roles")
